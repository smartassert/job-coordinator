...
use App\MessageFailureHandler\WorkerJobStartExceptionHandler;
...
class WorkerJobStartExceptionHandlerTest extends WebTestCase
{
    use RemoteRequestFailureCreationDataProviderTrait;
    use MockeryPHPUnitIntegration;

    private WorkerJobStartExceptionHandler $handler;
    private Job $job;
    ...
    protected function setUp(): void
    {
        ...
        $handler = self::getContainer()->get(WorkerJobStartExceptionHandler::class);
        \assert($handler instanceof WorkerJobStartExceptionHandler);
    ...
    public function testHandleSetRemoteRequestFailure(
        ...
        $this->handler->handle(new WorkerJobStartException($this->job, $throwable));
